...
public class RideServicesImpl implements RideServices {
		...
		try {
			...
			if (ride.getRideType().trim().equals(SyrRideType.PAID.Status())) {
				if (ride.getTotalFuelAmount() > SyrDefault.DOUBLE) {
					ride.setPerHeadAmount(SyrUtil.formatDouble(ride.getTotalFuelAmount() / ride.getSeatAvailability()));
				}
			}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.syr.services.RideServices#getMyRides(org.json.simple.JSONObject)
	 */
	...
	public Object getMyRides(JSONObject json) throws Exception {
		...
		try {
			...
			if (json.containsKey("pageNo")) {
				...
				direction = json.get("sortDirection").toString().trim().equals("A") ? Direction.ASC : Direction.DESC;
	...
	public Object inviteCoRiders(@Valid List<Invitation> invitations) throws Exception {
		try {
			...
			for (Object[] strArr : existingUsernamesArr) {
				existingUsernames.addAll(Arrays.asList(
						Arrays.stream(strArr).filter(s -> (s != null)).map(String::valueOf).toArray(String[]::new)));
			}
			...
			if (usernameList != null && !usernameList.isEmpty()) {
				...
				invitations = invitations.stream().peek(invite -> invite.setGuestUser(userList.stream()
						.filter(user -> (user.getPrimaryEmail().equals(invite.getGuestUsername().trim())
								|| String.valueOf(user.getPrimaryMobileNo()).equals(invite.getGuestUsername().trim())))
						.findAny().orElse(null)))
						.peek(invite -> invite.setRide(ride)).peek(invite->invite.setRideStatus(ride.getRideStatus()))
						.map(inviteRepo::save)
						.collect(Collectors.toList());
			}
		...
		try {
			...
			if (json.containsKey("pageNo")) {
				...
				direction = json.containsKey("sortDirection")
						? (json.get("sortDirection").toString().trim().equals("A") ? Direction.ASC : Direction.DESC)
						: Direction.ASC;
			}
			...
			case "B":
				...
				gInvJson.put("totalResultCount", pageNo > 0 ? gInvitesPage.getTotalElements() : guestInvites.size());
